...
num = linspace(8, 64, 8);
thickness_values = [0.0025,0.005, 0.01, 0.02];
all_errors = zeros(length(num), length(thickness_values));

for t_idx = 1:length(thickness_values)
    h = thickness_values(t_idx);
...
cnt=0;
for t_idx = 1:length(thickness_values)
    plot(num, all_errors(:, t_idx), '*-', 'DisplayName', sprintf('Thickness = %.4f', thickness_values(t_idx)));
end
